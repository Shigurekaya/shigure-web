  ...
  function attach(canvas, opts = {}) {
    ...
    const spawnGlassSplash = (opts = {}) => {
      ...
      const spl = {
        x,
        y,
        r: opts.r ?? rand(storm ? 0.55 : 0.65, lite ? 1.2 : (storm ? 1.6 : 1.8)),
        age: 0,
    ...
    const preseedGlassSplashes = () => {
      ...
      const cap = Math.min(Math.round(microN * (storm ? 0.1 : 0.22)), storm ? 48 : 72);
      for (let i = 0; i < cap; i += 1) {
        spawnGlassSplash({
          x: Math.random() * w,
          y: sampleSplashY(),
          r: rand(storm ? 0.45 : 0.55, storm ? 1.5 : 1.7),
        });
    ...
    const drawGlassSplashes = (aMul) => {
      for (let i = 0; i < glassSplashes.length; i += 1) {
        ...
        const spr = pickGlassSplashSpr(s.kind, rad * 1.8);
        if (spr) {
          const dwMul = storm ? (s.soft ? 1.65 : 1.35) : (s.soft ? 2.0 : 1.6);
          const dw = rad * dwMul;
          ctx.globalAlpha = s.a * aMul * p * (storm ? 0.72 : 0.88);
