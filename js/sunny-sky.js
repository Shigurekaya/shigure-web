...
(() => {
  ...
  function attach(host) {
    ...
    let deferredRainbowRebake = false;

    const isProfileAnimating = () => document.body.classList.contains("profile-animating");

    const flushDeferredWork = () => {
      if (!running || document.hidden || isProfileAnimating()) return;
      if (deferredRainbowRebake && rainbowTexReady && !bakedWithTex) {
        deferredRainbowRebake = false;
        bakedWithTex = true;
        refreshRainbowBake();
        bakeStaticScene();
      }
    ...
    const onProfileAnimEnd = () => {
      window.requestAnimationFrame(() => flushDeferredWork());
    };
    document.body.addEventListener("kaya:profile-open-change", onProfileAnimEnd);
    ...
    const applyResize = (force = false) => {
      ...
      /* 资料区展开/收回期间勿全屏 toDataURL，会阻塞主线程导致头像飞入卡住 */
    ...
    return {
      ...
      destroy() {
        ...
        document.body.removeEventListener("kaya:profile-open-change", onProfileAnimEnd);
        window.clearTimeout(resizeTimer);
        this.stop();
        sceneRevealed = false;
