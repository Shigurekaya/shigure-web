...
const Kaya = (() => {
  ...
  function initSandaimeBrand() {
    ...
    document.querySelectorAll(".site-header .brand").forEach((brand) => {
      applySandaimeMark(brand);
    });
  }

  function applySandaimeMark(brand) {
  ...
  function clearProfileInlineStyles() {
    document.querySelectorAll(
      ".profile-hero, .profile-avatar, .profile-name, .profile-roman, .profile-sign, .social-row, .profile-gated, .nav-gated, .social-pill, .home-card, .intro-panel, .home-stack",
    ).forEach((el) => {
      el.style.opacity = "";
      el.style.animation = "";
      el.style.transform = "";
    ...
    window.KayaProfileReveal?.repair?.();
  }
  ...
  function initProfileToggle() {
    ...
    brand.addEventListener("click", (e) => {
      ...
      const next = reveal?.isOpen ? !reveal.isOpen() : !document.body.classList.contains("profile-open");
      setProfileOpen(next);
    });
  ...
  function renderIntro(container) {
    ...
    } else if (intro && Array.isArray(intro.paragraphs) && intro.paragraphs.length) {
      html = intro.paragraphs.map((p) => `<p>${escapeHtml(p)}</p>`).join("");
    }
    container.innerHTML = html;
    container.closest(".intro-panel")?.classList.toggle("is-empty", !html);
  }
  ...
  function initFooterMotto() {
    ...
    function balanceBreak(text) {
      ...
      for (let i = 1; i < t.length - 1; i += 1) {
        if (!BREAK_MARKS.test(t[i])) continue;
        const after = i + 1;
      ...
      const left = t.slice(0, best + 1).trimEnd();
      const right = t.slice(best + 1).trimStart();
      if (!left || !right) return t;
      return `${left}\n${right}`;
    }

    function rawText(el) {
      return (el.dataset.mottoRaw || el.title || el.textContent || "")
        .replace(/\s+/g, " ")
        .trim();
    }
    ...
    function fitOne(el) {
      ...
      const broken = balanceBreak(text);
    ...
    const onReflow = () => {
      ...
      reflowTimer = window.setTimeout(fitAll, 100);
    };
    let reflowTimer = 0;
    ...
    void window.KayaFooterGalMotto?.applyRandomMotto?.();
  }
  ...
  const KAYA_ASSET_V = "202608281300";
  ...
  const SPA_MAIN = {
    ...
    <section class="intro-panel is-empty" aria-label="简介">
  ...
  function isScriptPathLoaded(pathKey) {
    return [...document.querySelectorAll("script[src]")].some((s) => {
      const attr = s.getAttribute("src") || "";
      return scriptPathKey(attr) === pathKey || attr.split("?")[0] === pathKey;
    });
  }
  ...
  /**
   * SPA 目标 URL：不自动粘贴天气参数。
   * 后缀仅来自左下角天气门 / 手写或写死的链接（目标自身已带 ?rain=… 时原样保留）。
   */
  ...
  function spaUrlHasWeatherOverride(url) {
    if ([...url.searchParams.keys()].some((k) => (
      ...
      || k === "sunny" || k === "snow" || k === "clear" || k === "rainbow" || k === "after"
    ))) return true;
    const p = normalizePathname(url.pathname);
    return p === "/heavy" || p === "/light" || p === "/storm" || p === "/sunny" || p === "/snow" || p === "/rainbow";
  }
  ...
  function loadScriptOnce(src) {
    ...
    const pathKey = scriptPathKey(src);
    if (isScriptPathLoaded(pathKey)) {
      loadedScripts.add(src);
      return Promise.resolve();
    }
    const pending = scriptLoadInflight.get(pathKey);
    ...
    const promise = new Promise((resolve, reject) => {
      ...
      s.onload = () => {
        loadedScripts.add(src);
        scriptLoadInflight.delete(pathKey);
        resolve();
      };
      s.onerror = () => {
        scriptLoadInflight.delete(pathKey);
    ...
    scriptLoadInflight.set(pathKey, promise);
    return promise;
  }
  ...
  async function softNavigate(page, { url, replace = false, fromPop = false, bypassGate = false } = {}) {
    if (!page || !SPA_MAIN[page]) return false;
    if (spaBusy) return null;
    if (!bypassGate && isProfileGatedHome() && PROFILE_GATED_PAGES.has(page)) return false;
  ...
  function initSpaRouter() {
    ...
    if (spaPage) {
      /* 只登记 history state，不改写地址（主域名保持干净，不自动加 ?rain=） */
      history.replaceState(
        { kayaSpa: spaPage },
        "",
        `${window.location.pathname}${window.location.search}${window.location.hash}`,
    ...
    document.addEventListener("click", (e) => {
      ...
      softNavigate(page, { url }).then((ok) => {
        /* null = busy（忽略）；false = 失败才整页跳 */
        if (ok === false) window.location.assign(url.href);
      });
    }, true);
    ...
    window.addEventListener("popstate", () => {
      ...
      /* 资料卡关闭时前进到受保护页：放行导航（bypass），避免 URL 与内容脱节 */
      softNavigate(page, {
        fromPop: true,
        bypassGate: isProfileGatedHome() && PROFILE_GATED_PAGES.has(page),
      });
    });
  ...
  function initSiteRain(host, rainMode) {
    ...
    const ensureSnowFx = () => {
      if (!bgCanvas) return null;
      if (snowFx) return snowFx;
      if (!window.KayaSnow?.attach) {
        console.warn("[kaya] KayaSnow missing");
        return null;
      }
      snowFx = window.KayaSnow.attach(bgCanvas, { mistHost: host });
      return snowFx;
    };
    ...
    const onResize = () => {
      ...
      /* 资料区动画期间跳过晴天 rebake，避免 toDataURL 卡住头像飞入 */
      if (dry && document.body.classList.contains("profile-animating")) return;
    ...
    if (heavy) {
      ...
      resize();
      /* 小雨开幕雨在 #intro-rain；主站层等开幕 teardown 后再 start，避免双引擎并行 */
      const introPlaying = document.body.classList.contains("home-intro-playing");
      if (introPlaying) {
        window.__kayaStartSiteRainAfterIntro = () => {
          delete window.__kayaStartSiteRainAfterIntro;
          startActiveFx();
        };
      } else {
        startActiveFx();
    ...
    rainApi = {
      ...
      destroy: () => {
        ...
        try { snowFx?.destroy?.(); } catch { /* ignore */ }
  ...
  function startIntroRain(canvas) {
    ...
    const snow = document.body.classList.contains("snow-sky");
    ...
    /* 雪开场：同页雪引擎，观感与主站一致 */
    if (snow) {
      if (window.KayaSnow?.attach && canvas) {
        const fx = window.KayaSnow.attach(canvas, { mistHost: null });
        fx.start();
        return () => {
  ...
  function shouldPlayHomeIntro(sitePrefix) {
    ...
    try {
      ...
      if (!prefix) {
        // Root site: treat /fuyuu and other creator paths as outside.
        if (
          p === "/fuyuu" ||
          p.startsWith("/fuyuu/") ||
          p.startsWith("/koharu") ||
  ...
  function weatherModeFromHref(href) {
    if (!href) return null;
    const q = href.match(/[?&]rain=(light|heavy|storm|sunny|snow|rainbow|after|clear)/i);
    if (q) return normalizeRainMode(q[1].toLowerCase());
    const p = href.match(/\/(light|heavy|storm|sunny|snow|rainbow)\/?$/i);
  ...
  function initWeatherPreview() {
    ...
    panel.querySelectorAll("a").forEach((a) => {
      a.addEventListener("click", (e) => {
        ...
        ["storm", "heavy", "light", "sunny", "snow", "clear", "rainbow", "after"].forEach((k) => {
  ...
  /** @param {"light"|"heavy"|"storm"|"sunny"|"snow"|boolean} modeOrHeavy */
  function applyRainTheme(modeOrHeavy) {
    ...
    document.body.classList.toggle("snow-sky", snow);
  ...
  function initHomeIntro() {
    ...
    const stormSession = document.body.classList.contains("storm-rain");
    const heavyOnlySession = document.body.classList.contains("heavy-rain") && !stormSession;
    /*
     * 入场：仅干净主域名上的随机天气（含晴/雪/小雨/大雨）。
     * 左下角或写死后缀（?rain=… / /storm 等）一律不播。
     */
    const play = !forceSpecialWeatherFromUrl() && shouldPlayHomeIntro("/");

    if (!intro || reduced || !play || (phoneLike && heavyOnlySession)) {
    ...
    const finishIntro = (force = false) => {
      ...
      if (force) {
        ...
        document.body.classList.remove("home-intro-playing");
        stopRain();
        try { window.__kayaStartSiteRainAfterIntro?.(); } catch { /* ignore */ }
