...
const Shiotsuki = (() => {
  ...
  function syncHeaderHeights() {
    ...
    root.style.setProperty("--st-wip-h", "0px");
    root.style.setProperty("--st-site-head-h", `${topbarH}px`);
  }
  ...
  function appendWorksBatch(grid, videos) {
    if (!grid || worksShown >= videos.length) return;
    const next = Math.min(worksShown + WORKS_BATCH, videos.length);
    const frag = document.createDocumentFragment();
    for (let i = worksShown; i < next; i++) {
      frag.appendChild(createWorkCard(videos[i], i));
    }
    grid.appendChild(frag);
    worksShown = next;
  }

  function initWorksInfiniteScroll(videos) {
    ...
    grid.innerHTML = "";
    worksShown = 0;
    document.getElementById("works-load-more")?.remove();
  ...
  function initWorks() {
    ...
    initWorksInfiniteScroll(videos);
