...
const Koharu = (() => {
  ...
  function spaceUrl() {
    const d = data();
    return d.bilibili_url || `https://space.bilibili.com/${d.mid || 228092838}`;
  }
  ...
  function renderGrid(grid, videos, startIndex = 0) {
    ...
    grid.innerHTML = "";
    if (!videos.length) {
      grid.innerHTML = '<p class="kn-empty">暂无作品数据</p>';
      return;
    }
  ...
  function renderSeasons(container, seasons, limit) {
    if (!container) return;
    const section = container.closest(".kn-section");
    const prevHead = container.previousElementSibling;
    const list = limit ? (seasons || []).slice(0, limit) : seasons || [];
    if (!list.length) {
      if (section) section.hidden = true;
      if (prevHead?.classList.contains("kn-section-head")) prevHead.hidden = true;
      container.hidden = true;
      return;
    }
    if (section) section.hidden = false;
    if (prevHead?.classList.contains("kn-section-head")) prevHead.hidden = false;
  ...
  function initNav() {
    const inner = document.querySelector(".kn-topbar-inner");
    const nav = document.querySelector(".kn-nav");
    if (!inner || !nav || document.getElementById("kn-menu-toggle")) return;

    const mobileMq = window.matchMedia("(max-width: 768px)");
    const logoMark = document.querySelector(".kn-logo-mark")?.textContent || "春";

    const backdrop = document.createElement("div");
    backdrop.className = "kn-menu-backdrop";
    backdrop.hidden = true;
    document.body.appendChild(backdrop);

    const drawerHead = document.createElement("div");
    drawerHead.className = "kn-drawer-head";
    drawerHead.innerHTML =
      `<span class="kn-drawer-logo" aria-hidden="true">${escapeHtml(logoMark)}</span>`
      + '<button type="button" class="kn-drawer-close" aria-label="关闭菜单">×</button>';
    nav.insertBefore(drawerHead, nav.firstChild);
    nav.id = "kn-site-nav";
    nav.classList.add("kn-mobile-drawer");

    const btn = document.createElement("button");
    btn.type = "button";
    btn.id = "kn-menu-toggle";
    btn.className = "kn-menu-toggle";
    btn.setAttribute("aria-label", "菜单");
    btn.setAttribute("aria-expanded", "false");
    btn.setAttribute("aria-controls", "kn-site-nav");
    btn.innerHTML =
      '<span class="kn-menu-toggle-icon" aria-hidden="true"><span></span><span></span><span></span></span>';
    ...
    const closeMenu = () => {
      ...
      btn.setAttribute("aria-expanded", "false");
      document.body.classList.remove("kn-menu-open");
    ...
    const openMenu = () => {
      ...
      requestAnimationFrame(() => {
        ...
        document.body.classList.add("kn-menu-open");
      });
    };
    ...
    drawerHead.querySelector(".kn-drawer-close")?.addEventListener("click", (e) => {
      e.stopPropagation();
      closeMenu();
    });
  ...
  function syncHeaderHeights() {
    const wip = document.querySelector(".kn-wip");
    const topbar = document.querySelector(".kn-topbar");
    const wipH = wip?.offsetHeight ?? 0;
    const topbarH = topbar?.offsetHeight ?? 0;
    const root = document.documentElement;
    root.style.setProperty("--kn-wip-h", `${wipH}px`);
    root.style.setProperty("--kn-site-head-h", `${wipH + topbarH}px`);
  }

  function initWipBanner() {
    if (document.getElementById("koharu-wip")) return;
    const topbar = document.querySelector(".kn-topbar");
    if (!topbar) return;
    const banner = document.createElement("p");
    banner.id = "koharu-wip";
    banner.className = "kn-wip";
  ...
  function renderDataSyncNote() {
    const footer = document.querySelector(".kn-footer");
    const updated = data().data_updated;
    if (!footer || !updated) return;
    let el = footer.querySelector(".kn-data-note");
    if (!el) {
      el = document.createElement("p");
      el.className = "kn-data-note";
  ...
  function updateWorksLoadMore(videos) {
    ...
    if (!btn) {
      ...
      btn.className = "kn-btn kn-btn--ghost kn-load-more";
